...
class Clonecard31 extends Card
{
    ...
    public function init($parg1, $parg2, $varg1, $varg2)
    {
        ...
        //if($varg1 == "confirm")
        {
            ...
            spellbook::$instance->notifyAllPlayers('message',clienttranslate( '${player_name} triggers "BLAZE" (Clone)'), array(
                'player_name' => $this->player_name,
            ...
            if ($nbreplayer==1)
            {
                spellbook::$instance->addPendingTarget($this->player_id, "Clonecard31", "TakeOtherSolo");
            }
            ...
            if ($nbreplayer==2)
            {
                if ($this->player_no == 1)
                {
                    ...
                    spellbook::$instance->addPendingTarget($id2, "Clonecard31", "TakeOther", $id2);
                }
                if ($this->player_no == 2)
                {
                    $id1 = intval(self::getUniqueValueFromDB("SELECT `player_id` FROM `player` WHERE `player_no` = 1"));
                    spellbook::$instance->addPendingTarget($id1, "Clonecard31", "TakeOther", $id1);
                }
            ...
            if ($nbreplayer==3)
            {
                if ($this->player_no == 1)
                {
                    ...
                    spellbook::$instance->addPendingTarget($id3, "Clonecard31", "TakeOther", $id3);
                    $id2 = intval(self::getUniqueValueFromDB("SELECT `player_id` FROM `player` WHERE `player_no` = 2"));
                    spellbook::$instance->addPendingTarget($id2, "Clonecard31", "TakeOther", $id2);
            ...
            if ($nbreplayer==4)
            {
                if ($this->player_no == 1)
                {
                    ...
                    spellbook::$instance->addPendingTarget($id4, "Clonecard31", "TakeOther", $id4);
                    $id3 = intval(self::getUniqueValueFromDB("SELECT `player_id` FROM `player` WHERE `player_no` = 3"));
                    spellbook::$instance->addPendingTarget($id3, "Clonecard31", "TakeOther", $id3);
                    $id2 = intval(self::getUniqueValueFromDB("SELECT `player_id` FROM `player` WHERE `player_no` = 2"));
                    spellbook::$instance->addPendingTarget($id2, "Clonecard31", "TakeOther", $id2);
    ...
    public function TakeOther($parg1, $parg2, $varg1, $varg2)
    {
        if($varg1 != NULL)
        {
            spellbook::$instance->addPendingTarget($parg1, "Clonecard31", "Confirm", $parg1, $varg1);
        }
    ...
    public function Confirm($parg1, $parg2, $varg1, $varg2)
    {
        ...
        {
            spellbook::$instance->addPendingTarget($parg1, "Clonecard31", "TakeOther", $parg1);
        }
    ...
    public function TakeOtherSolo($parg1, $parg2, $varg1, $varg2)
    {
        if($varg1 != NULL)
        {
            spellbook::$instance->addPendingTarget($this->player_id, "Clonecard31", "ConfirmSolo", $varg1);
        }
