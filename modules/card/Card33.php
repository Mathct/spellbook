...
class Card33 extends Card
{
    public function arginit($parg1, $parg2)
    {
        ...
        $level = intval(self::getUniqueValueFromDB("SELECT `power` FROM `cards` WHERE `player_id`={$this->player_id} AND `set_color` = 3 AND `typerune` !=0")); //////ATTENTION set_color
    ...
    public function init($parg1, $parg2, $varg1, $varg2)
    {
        ...
        else
        {
            if($varg1 == NULL)
            {
                ...
                if ($level == 1)
                {
                    spellbook::$instance->addPendingTarget($this->player_id, "Card33", "Swap");
                }
        
                if ($level == 2)
                {
                    spellbook::$instance->addPendingTarget($this->player_id, "Card33", "AltarStore2");
                }
        
                if ($level == 3)
                {
                    spellbook::$instance->addPendingTarget($this->player_id, "Card33", "AltarStore3");
                }
            }
    ...
    public function Swap($parg1, $parg2, $varg1, $varg2)
    {
        ...
        {
            spellbook::$instance->addPendingTarget($this->player_id, "Card33", "SwapStep2", $varg1);
        }
    ...
    public function SwapStep2($parg1, $parg2, $varg1, $varg2)
    {
        ...
        {
            spellbook::$instance->addPendingTarget($this->player_id, "Card33", "SwapConfirm", $parg1, $varg1);
        }
    ...
    public function SwapConfirm($parg1, $parg2, $varg1, $varg2)
    {
        ...
        {*/
            ...
            spellbook::$instance->notifyAllPlayers('message',clienttranslate( '${player_name} triggers "GROWTH" and swaps ${log1} from the reserve with ${log2} from the familiar'), array(
    ...
    public function AltarStore2Confirm($parg1, $parg2, $varg1, $varg2)
    {
        ...
        {*/
            ...
            spellbook::$instance->notifyAllPlayers('message',clienttranslate( '${player_name} triggers "GROWTH" and stores ${log1} ${log2} from the Altar'), array(
                'player_name' => $this->player_name,
                'log1' => spellbook::$instance->getLogsType($log[0]),
                'log2' => spellbook::$instance->getLogsType($log[1]),
            ...
            spellbook::$instance->addPendingTarget($this->player_id, "Card33", "Level");
        //}
    ...
    public function AltarStore3Confirm($parg1, $parg2, $varg1, $varg2)
    {
        ...
        {*/
            ...
            for ($i=1; $i <= 3; $i++)
            {
                ...
                spellbook::$instance->notifyAllPlayers('message',clienttranslate( '${player_name} triggers "GROWTH" and stores ${log1} ${log2} ${log3} from the Altar'), array(
                    'player_name' => $this->player_name,
                    'log1' => spellbook::$instance->getLogsType($log[0]),
                    'log2' => spellbook::$instance->getLogsType($log[1]),
    ...
    {
        $materiacard = 'materiacard_3_'.$this->player_id;
        ...
        $power = intval(self::getUniqueValueFromDB("SELECT `power` FROM `cards` WHERE `player_id`={$this->player_id} AND `set_color` = 3 AND `typerune` != 0"));
        $rune = intval(self::getUniqueValueFromDB("SELECT `typerune` FROM `cards` WHERE `player_id`={$this->player_id} AND `set_color` = 3 AND `typerune` != 0"));

        $powerdown = $power-1;

        self::DbQuery( "UPDATE `cards` set `typerune` = 0 WHERE `set_color` = 3 AND `power` = {$power} AND `player_id` = {$this->player_id}" );
        self::DbQuery( "UPDATE `cards` set `typerune` = {$rune} WHERE `set_color` = 3 AND `power` = {$powerdown} AND `player_id` = {$this->player_id}" );
